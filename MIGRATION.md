...
5. **ایمنی در محیط چندنخی (Thread-Safe):** تمامی اعتبارسنج‌ها بدون حالت سراسری هستند؛ `PassportValidator.Instance` همیشه strict است و برای داده آرشیوی از `PassportValidator.CreateArchiveValidator()` یا opt-in محلی wrapperها استفاده کنید.
6. **کنترل کاراکترهای bidi:** کاراکترهای کنترل جهت در validatorها رد می‌شوند؛ در صورت نیاز به نرمال‌سازی bidi، آن را در لایه پذیرش ورودی و قبل از ذخیره‌سازی انجام دهید.
