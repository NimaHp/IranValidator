...
* **Unicode Controls:** Bidi controls are not removed by the validator pipeline and cause rejection; known zero-width and format characters are normalized through the central character list.
* **Local, Thread-Safe Configuration:** `PassportValidator.Instance` is always strict; legacy 8-digit archive support is enabled only on the immutable instance returned by `PassportValidator.CreateArchiveValidator()`. DataAnnotations and FluentValidation wrappers keep the opt-in local to each instance, with no mutable global state.
